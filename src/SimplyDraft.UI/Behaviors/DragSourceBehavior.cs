...
public sealed class DragSourceBehaviour : Behavior<Control>
{
    ...
    public static readonly StyledProperty<string?> PayloadProperty =
        AvaloniaProperty.Register<DragSourceBehaviour, string?>(nameof(Payload));
    
    public static readonly StyledProperty<double> ThresholdProperty =
        AvaloniaProperty.Register<DragSourceBehaviour, double>(nameof(Threshold), 4.0);
    ...
    private async void OnMoved(object? sender, PointerEventArgs e)
    {
        ...
        try
        {
            await DragDrop.DoDragDropAsync(pressed, data, DragDropEffects.None);
        }
        catch
        {
            // Nothing to clean up
