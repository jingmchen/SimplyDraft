...
public sealed partial class Library : ILibrary
{
    ...
    public List<LibraryItem> Scan()
    {
        ...
        foreach (var f in EnumerateFilesSafe(_paths.TemplatesFolder, InfrastructureConstants.FileExtension.Template))
            items.Add(ScanSingle(f, LibraryItemKind.Template));
        
        foreach (var f in EnumerateFilesSafe(_paths.ChildrenFolder, InfrastructureConstants.FileExtension.Children))
    ...
    private string? ResolveInputName(string name)
    {
        string bare = name.EndsWith(
            InfrastructureConstants.FileExtension.Template, StringComparison.OrdinalIgnoreCase
        ) ? name[..^4] : name;
        
        try
        {
            string direct = Path.Combine(_paths.TemplatesFolder, bare + InfrastructureConstants.FileExtension.Template);
