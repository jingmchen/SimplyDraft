...
public static class FrontMatterParser
{
        ...
        for (int lineIndex = 1; lineIndex < closingFenceLine; lineIndex++)
        {
            ...
            if (indent == 0)
            {
                ...
                switch (key.ToLowerInvariant())
                {
                    ...
                    case "name":
                        frontMatter.Name = ReadScalar(rawValue, indent, ref lineIndex);
                        break;
                    
                    case "description":
                        frontMatter.Description = ReadScalar(rawValue, indent, ref lineIndex);
                        break;
                    
                    case "template":
                        frontMatter.TemplatePath = ReadScalar(rawValue, indent, ref lineIndex);
                        break;
                    
                    case "markup":
                        frontMatter.HasMarkup = ParseMarkupFlag(ReadScalar(rawValue, indent, ref lineIndex));
                        break;
            ...
            else
            {
                ...
                switch (currentMap)
                {
                    ...
                    case "types":
                        frontMatter.Types[key] = value.Trim().ToLowerInvariant();
                        break;
                    
                    case "docx":
                        if (key.Equals("font", StringComparison.OrdinalIgnoreCase))
                            frontMatter.DocxFont = value;
                        else if (key.Equals("size", StringComparison.OrdinalIgnoreCase) && int.TryParse(value, out var sizePoints))
                            frontMatter.DocxSizePt = sizePoints;
                        else if (key.Equals("header", StringComparison.OrdinalIgnoreCase))
                            frontMatter.DocxHeader = value;
                        else
                            warnings.Add(Warn($"unknown docx setting '{key}' ignored", lineIndex + 1));
    ...
    public static string Write(FrontMatter frontMatter, string body)
    {
        ...
        if (frontMatter.Name != null)
            Scalar("name", frontMatter.Name, 0);
        
        if (frontMatter.Description != null)
            Scalar("description", frontMatter.Description, 0);
        
        if (frontMatter.TemplatePath != null)
            Scalar("template", frontMatter.TemplatePath, 0);
        
        if (frontMatter.HasMarkup)
            builder.Append("markup: true\n");
        
        if (frontMatter.Variables.Count > 0)
        {
            builder.Append("variables:\n");
            foreach (var entry in frontMatter.Variables)
                Scalar(entry.Key, entry.Value, 2);
        }

        if (frontMatter.Types.Count > 0)
        {
            builder.Append("types:\n");
            foreach (var entry in frontMatter.Types)
                Scalar(entry.Key, entry.Value, 2);
        }
        ...
        if (frontMatter.Values.Count > 0)
        {
            ...
            foreach (var entry in frontMatter.Values)
                Scalar(entry.Key, entry.Value, 2);
        }
        ...
        if (frontMatter.DocxFont != null || frontMatter.DocxSizePt != null || frontMatter.DocxHeader != null)
        {
            ...
            if (frontMatter.DocxFont != null)
                Scalar("font", frontMatter.DocxFont, 2);
            
            if (frontMatter.DocxSizePt is int sizePoints)
                builder.Append("  size: ").Append(sizePoints).Append('\n');
            
            if (frontMatter.DocxHeader != null)
                Scalar("header", frontMatter.DocxHeader, 2);
        }

        foreach (var entry in frontMatter.Extras)
            Scalar(entry.Key, entry.Value, 0);
