...
public sealed class Interpreter
{
    ...
    private static int CompareTemporal(Value left, Value right, int line, int column)
    {
        if (left.Kind == ValueKind.Time || right.Kind == ValueKind.Time)
            return ToTime(left, line, column).CompareTo(ToTime(right, line, column));
        
        return ToDateTime(left, line, column).CompareTo(ToDateTime(right, line, column));
    }

    private static TimeSpan ToTime(Value value, int line, int column) => value.Kind switch
    {
        ...
        ValueKind.Str => throw ScriptException.Error(
            DiagnosticCode.TemporalParse, $"\"{value.AsString}\" is not a time — expected HH:mm or HH:mm:ss", line, column),

        _ => throw ScriptException.Error(DiagnosticCode.TypeMismatch, $"cannot compare a time with a {value.KindName}", line, column)
    };

    private static DateTime ToDateTime(Value value, int line, int column) => value.Kind switch
    {
