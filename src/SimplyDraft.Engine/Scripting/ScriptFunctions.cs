...
public static class ScriptFunctions
{
    ...
    private static readonly Dictionary<string, Fn> Map = new(StringComparer.Ordinal)
    {
        [ScriptingConstants.Functions.Len] = new(1, 1, (a, cu, l, c) => a[0].Kind == ValueKind.Str
            ...
            : throw ScriptException.Error(
                ...
                $"len() expects a str, got {a[0].KindName}",
                l, c)),
        ...
        [ScriptingConstants.Functions.Float] = new(1, 1, (a, cu, l, c) => a[0].Kind switch
        {
            ...
            ValueKind.Str => double.TryParse(a[0].AsString.Trim(), NumberStyles.Float, CultureInfo.InvariantCulture, out var d)
                ? Value.Num(d)
                : throw ScriptException.Error(DiagnosticCode.TypeMismatch, $"could not convert str to float: \"{a[0].AsString.Trim()}\"", l, c),
            
            _ => throw ScriptException.Error(DiagnosticCode.TypeMismatch, $"float() expects a str or number, got {a[0].KindName}", l, c)
        }),
        ...
        [ScriptingConstants.Functions.Format] = new(2, 2, (a, cu, l, c) =>
        {
            if (a[1].Kind != ValueKind.Str)
                throw ScriptException.Error(
                    DiagnosticCode.TypeMismatch, $"format() second argument must be a str format, got {a[1].KindName}", l, c);
            ...
            try
            {
                return a[0].Kind switch
                {
                    ...
                    _ => throw ScriptException.Error(
                        DiagnosticCode.TypeMismatch, $"format() supports numbers and date/time values, got {a[0].KindName}", l, c)
                };
            }
            catch (FormatException)
            {
                throw ScriptException.Error(
                    DiagnosticCode.TypeMismatch, $"format() format string \"{fmt}\" is invalid", l, c);
            }
        }),
    };
    ...
    public static Value Invoke(string name, Value[] args, CultureInfo formatCulture, int line, int column)
    {
        if (!Map.TryGetValue(name, out var fn))
        {
            ...
            if (canonical != null)
                throw ScriptException.Error(
                    DiagnosticCode.UnknownFunction, $"unknown function '{name}()' — did you mean '{canonical}()'? (names are case-sensitive)", line, column);
            
            if (Hints.TryGetValue(name, out var hint))
                throw ScriptException.Error(
                    DiagnosticCode.UnknownFunction, $"unknown function '{name}()' — {hint}", line, column);
            
            throw ScriptException.Error(
                DiagnosticCode.UnknownFunction, $"unknown function '{name}()' — available: len(), str(), float(), format()", line, column);
        }
        ...
        if (args.Length < fn.Min || args.Length > fn.Max)
        {
            ...
            throw ScriptException.Error(
                DiagnosticCode.WrongArity, $"{name}() takes {expected} argument(s), got {args.Length}", line, column);
        }
        
        return fn.Impl(args, formatCulture, line, column);
    }
}