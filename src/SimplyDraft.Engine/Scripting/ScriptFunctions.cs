...
public static class ScriptFunctions
{
    ...
    public static Value Invoke(string name, Value[] args, CultureInfo formatCulture, int line, int col)
    {
        if (!Map.TryGetValue(name, out var fn))
        {
            ...
            if (canonical != null)
                throw ScriptException.Error(
                    DiagnosticCode.UnknownFunction, $"unknown function '{name}()' — did you mean '{canonical}()'? (names are case-sensitive)", line, col
                );
            
            if (Hints.TryGetValue(name, out var hint))
                throw ScriptException.Error(
                    DiagnosticCode.UnknownFunction, $"unknown function '{name}()' — {hint}", line, col
                );
            
            throw ScriptException.Error(
                DiagnosticCode.UnknownFunction, $"unknown function '{name}()' — available: len(), str(), float(), format()", line, col
            );
        }
        ...
        if (args.Length < fn.Min || args.Length > fn.Max)
        {
            ...
            throw ScriptException.Error(
                DiagnosticCode.WrongArity, $"{name}() takes {expected} argument(s), got {args.Length}", line, col
            );
        }
