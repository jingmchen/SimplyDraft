...
public sealed class MarkupParser
{
    ...
    public MarkupDocument Parse(string text)
    {
        ...
        for (int index = 0; index < lines.Length; index++)
        {
            ...
            if (_openTable != null)
            {
                TakeTableLine(trimmed, lineNumber);
                continue;
            }

            if (_openFigure != null)
            {
                TakeFigureLine(trimmed, lineNumber);
                continue;
            }

            if (TryWholeLineCommand(trimmed, lineNumber))
                continue;
    ...
    private bool TryWholeLineCommand(string trimmed, int lineNumber)
    {
        switch (trimmed)
        {
            case MarkupConstants.Tokens.BeginVerbatim:
                _insideVerbatim = true;
                _lastListItem = null;
                return true;
            
            case MarkupConstants.Tokens.BeginItemize:
                _openLists.Add((ListKind.Bullet, 0));
                _lastListItem = null;
                return true;
            
            case MarkupConstants.Tokens.BeginEnumerate:
                _openLists.Add((ListKind.Number, 0));
                _lastListItem = null;
                return true;
            
            case MarkupConstants.Tokens.BeginDescription:
                _openLists.Add((ListKind.Description, 0));
                _lastListItem = null;
                return true;
            ...
            case MarkupConstants.Tokens.EndDescription:
                if (_openLists.Count > 0)
                    _openLists.RemoveAt(_openLists.Count - 1);
                else
                    Warn($"unmatched {trimmed}", lineNumber);

                _lastListItem = null;
                return true;

            case MarkupConstants.Tokens.BeginQuote:
                _quoteDepth++;
                _lastListItem = null;
                return true;
            ...
            case MarkupConstants.Tokens.EndQuote:
                if (_quoteDepth > 0)
                    ...
                    Warn("unmatched \\end{quote}", lineNumber);
                
                _lastListItem = null;
                return true;

            case MarkupConstants.Tokens.BeginCenter:
                _centerDepth++;
                _lastListItem = null;
                return true;
            ...
            case MarkupConstants.Tokens.EndCenter:
                if (_centerDepth > 0)
                    ...
                    Warn("unmatched \\end{center}", lineNumber);
            ...
            case MarkupConstants.Tokens.EndFigure:
                Warn("unmatched \\end{figure}", lineNumber);
                return true;

            case MarkupConstants.Tokens.EndTabular:
                Warn("unmatched \\end{tabular}", lineNumber);
                return true;
            ...
            case MarkupConstants.Tokens.ClearPage:
                _document.Blocks.Add(new PageBreakBlock());
                _lastListItem = null;
                return true;
            
            case MarkupConstants.Tokens.HRule:
            case MarkupConstants.Tokens.HLine: // stray \hline ≈ rule
                _document.Blocks.Add(new RuleBlock());
                _lastListItem = null;
                return true;
            
            case MarkupConstants.Tokens.TableOfContents:
                _document.Blocks.Add(new TableOfContentsBlock());
                _lastListItem = null;
                return true;
            
            case MarkupConstants.Tokens.ListOfFigures:
                _document.Blocks.Add(new ListOfFiguresBlock());
                _lastListItem = null;
                return true;
            
            case MarkupConstants.Tokens.MakeTitle:
                EmitTitleBlock(lineNumber);
                return true;
            ...
            case MarkupConstants.Tokens.NoIndent:
                return true; // paragraphs are not indented — accepted, no effect
    ...
    private bool TryHeading(string trimmed, int lineNumber)
    {
        foreach (var (command, level) in HeadingCommands)
        {
            ...
            if (position >= trimmed.Length || trimmed[position] != MarkupConstants.Delimiters.GroupOpen)
                continue;
            ...
            var heading = new ParagraphBlock(
                level switch
                {
            ...
            foreach (var key in labels)
                BindLabel(key, starred ? _currentAnchor : headingNumber, lineNumber);
            
            if (trailingText.Length > 0)
            {
                var tail = new ParagraphBlock(
                    _quoteDepth > 0 ? ParagraphKind.Quote : ParagraphKind.Normal,
                    centered: _centerDepth > 0);
    ...
    private static (List<string> Labels, int After) ReadLeadingLabels(string trimmed, int from)
    {
        ...
        while (true)
        {
            while (scan < trimmed.Length && char.IsWhiteSpace(trimmed[scan]))
                scan++;

            if (scan >= trimmed.Length)
                break;
            
            if (!trimmed.AsSpan(scan).StartsWith(MarkupConstants.Tokens.Label, StringComparison.Ordinal))
                break;
            
            int braceOpen = scan + MarkupConstants.Tokens.Label.Length;
            
            if (braceOpen >= trimmed.Length || trimmed[braceOpen] != MarkupConstants.Delimiters.GroupOpen)
                break;
    ...
    private bool TryRunInHeading(string trimmed, int lineNumber)
    {
        foreach (var command in new[] {MarkupConstants.Tokens.Subparagraph, MarkupConstants.Tokens.Paragraph})
        {
            ...
            if (position < trimmed.Length && trimmed[position] == MarkupConstants.Delimiters.StarredSuffix)
                position++;
    ...
    private bool TryTitleMeta(string trimmed, int lineNumber)
    {
        ...
        {
            if (!trimmed.StartsWith(command + MarkupConstants.Delimiters.GroupOpen, StringComparison.Ordinal))
                continue;
            ...
            switch (command)
            {
                case MarkupConstants.Tokens.Title:
                    _titleInlines = inlines;
                    break;
                
                case MarkupConstants.Tokens.Author:
                    _authorInlines = inlines;
                    break;
                
                default:
                    _dateInlines = inlines;
                    _dateGiven = true; 
                    break; // \date{} suppresses the date
            }
    ...
    private string BumpHeadingNumber(int level)
    {
        switch (level)
        {
            case 1:
                ...
                _heading2Count = 0;
                _heading3Count = 0;
                return _heading1Count.ToString(CultureInfo.InvariantCulture);
            
            case 2:
                _heading2Count++;
                _heading3Count = 0;
                return $"{_heading1Count}.{_heading2Count}";
            
            default:
                _heading3Count++;
                return $"{_heading1Count}.{_heading2Count}.{_heading3Count}";
        }
    }
    ...
    private bool TryListItem(string trimmed, int lineNumber)
    {
        bool isItemLine = _openLists.Count > 0
            ...
            && (trimmed.Length == MarkupConstants.Tokens.Item.Length || !char.IsLetter(trimmed[MarkupConstants.Tokens.Item.Length]));
    ...
    {
        if (_openLists.Count == 0 || _lastListItem is null)
            return false;
        ...
        if (rest.StartsWith(MarkupConstants.Delimiters.GroupOpen))
        {
            ...
            if (close > 0)
            {
                foreach (char specifier in rest[1..close])
                    if (specifier is MarkupConstants.ColumnAlignment.AlignLeft or MarkupConstants.ColumnAlignment.AlignCenter
    ...
    {
        if (trimmed == MarkupConstants.Tokens.EndTabular)
        {
            CloseTable(lineNumber);
            return;
        }
        ...
        if (trimmed.Length == 0 || trimmed == MarkupConstants.Tokens.HLine)
            return;
    ...
    {
        if (_openTable!.Rows.Count > 0)
            _document.Blocks.Add(_openTable);
        else
            Warn("empty tabular environment", lineNumber);
    ...
    {
        if (trimmed == MarkupConstants.Tokens.EndFigure)
        {
            EmitFigure(lineNumber);
            return;
        }
        ...
        if (trimmed == MarkupConstants.Tokens.Centering)
        {
            _openFigure!.Centered = true;
            return;
        }

        if (TryReadIncludeGraphics(trimmed, out var imagePath))
        {
            if (_openFigure!.Path.Length > 0)
                Warn("second \\includegraphics in the same figure is ignored", lineNumber);
            else
                _openFigure.Path = imagePath;
        ...
        {
            if (_openFigureHasCaption)
            {
                Warn("second \\caption in the same figure is ignored", lineNumber);
            }
            else
            {
            ...
            foreach (var key in captionLabels)
                BindLabel(key, CurrentLabelTarget, lineNumber);
    ...
    private static bool TryReadCaption(string trimmed, out string captionText, out List<string> labels)
    {
        ...
        if (!trimmed.StartsWith(MarkupConstants.Tokens.CaptionOpen, StringComparison.Ordinal))
            return false;
    ...
    {
        if (!TryReadCaption(trimmed, out _, out _))
            return false;
    ...
    {
        if (TrailingLabels(trimmed, 0) is not {Count: > 0} labels)
            return false;

        foreach (var key in labels)
            BindLabel(key, _currentAnchor, lineNumber);
    ...
    {
        if (_insideVerbatim)
            Warn("\\begin{verbatim} is never closed", lastLineNumber);
        
        if (_openTable != null)
        {
            Warn("\\begin{tabular} is never closed", lastLineNumber);
            CloseTable(lastLineNumber);
        }

        if (_openFigure != null)
        {
            Warn("\\begin{figure} is never closed", lastLineNumber);
            EmitFigure(lastLineNumber);
        }

        if (_openLists.Count > 0)
            Warn("a list environment is never closed", lastLineNumber);
        
        if (_quoteDepth > 0)
            Warn("\\begin{quote} is never closed", lastLineNumber);
        
        if (_centerDepth > 0)
            Warn("\\begin{center} is never closed", lastLineNumber);
    ...
    private void ResolveReferences()
    {
        foreach (var inlineList in AllInlineLists())
            ...
            {
                if (inlineList[index] is not RefRun reference)
                    continue;

                if (_labels.TryGetValue(reference.Key, out var target) && target.Length > 0)
                {
                    inlineList[index] = new TextRun(
                        target, reference.Bold, reference.Italic, reference.Underline, reference.Mono);
                }
                else
                {
                    ...
                    inlineList[index] = new TextRun(
                        "?" + reference.Key + "?", reference.Bold, reference.Italic, reference.Underline, reference.Mono);
                }
            }
    }
