...
internal sealed class InlineParser
{
    ...
    private readonly MarkupParser _mainParser;

    internal InlineParser(MarkupParser host)
        => _mainParser = host ?? throw new ArgumentNullException(nameof(host));
        ...
        int depth)
    {
        ...
        {
            _mainParser.Warn($"formatting nested deeper than {MaxInlineDepth} levels — kept as plain text", lineNumber);

            if (end > start)
                output.Add(MakeTextRun(source[start..end], style));
        ...
        int depth)
    {
        switch (command)
        {
            ...
            case MarkupConstants.Commands.Today:
                pendingText.Append(_mainParser.Today.ToString(MarkupConstants.Formats.DateFormat, _mainParser.Culture));
                return commandEnd;
            ...
            case MarkupConstants.Commands.HSpace:
                return ConsumeSpaceCommand(
                    source, end, command, commandEnd, output, pendingText, flush);
            ...
            case MarkupConstants.Commands.Ref:
                return ConsumeRefCommand(
                    source, end, commandEnd, style, output, pendingText, flush, lineNumber);

            case MarkupConstants.Commands.Label:
                return ConsumeLabelCommand(
                    source, end, commandEnd, pendingText, lineNumber);
            
            default:
                _mainParser.Warn(
                    LineOnlyCommands.Contains(command)
        ...
        int depth)
    {
        ...
        _mainParser.Warn($"\\{command} expects an argument in braces", lineNumber);
        pendingText.Append(MarkupConstants.Delimiters.SyntaxStart).Append(command);
        return commandEnd;
    }
        ...
        int lineNumber)
    {
        ...
        _mainParser.Warn("\\ref expects a label name in braces", lineNumber);
        ...
        int lineNumber)
    {
        if (commandEnd < end && source[commandEnd] == MarkupConstants.Delimiters.GroupOpen)
        {
            ...
            if (close > commandEnd)
            {
                _mainParser.BindLabel(source[(commandEnd + 1)..close].Trim(), _mainParser.CurrentLabelTarget, lineNumber);
                return close + 1;
            }
        }

        _mainParser.Warn("\\label expects a name in braces", lineNumber);
