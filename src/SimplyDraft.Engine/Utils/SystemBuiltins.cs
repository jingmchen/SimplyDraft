...
{
    public static Value? Lookup(BuiltinContext context, string ns, string member)
    {
    ...
    private static Value? LookupSystem(DateTime now, string member)
        => member switch
        {
    ...
    private static Value? LookupDoc(DocInfo doc, string member)
        => member switch
        {
