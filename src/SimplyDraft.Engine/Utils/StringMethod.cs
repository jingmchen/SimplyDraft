...
{
    /// <summary>FR-SCR-03 hard limit — keeps rjust/ljust from allocating pathological strings.</summary>
    ...
    private static readonly Dictionary<string, Method> Map = new(StringComparer.Ordinal)
    {
        ...
        [ScriptingConstants.Methods.Replace] = new(2, 2, (s, a, l, c) =>
        {
            ...
            if (find.Length == 0) return Value.Str(s);
            // Guard the same memory budget as '+': a growing replacement (repl longer than find) across
            // many matches can balloon the string, so project the size and reject BEFORE allocating.
            if (repl.Length > find.Length)
            {
                ...
                if (projected > Interpreter.MaxStringLength)
                    throw ScriptException.Error(DiagnosticCode.LimitExceeded,
                        $"replace() would exceed the maximum string length ({Interpreter.MaxStringLength:N0} characters)", l, c);
            }
    ...
    public static Value Invoke(Value receiver, string name, Value[] args, int line, int col)
    {
        ...
        if (receiver.Kind != ValueKind.Str)
            throw ScriptException.Error(DiagnosticCode.TypeMismatch,
                $".{name}() works on str values, got {receiver.KindName} — wrap it with str(…)", line, col);
        ...
        return method.Impl(receiver.AsString, args, line, col);
    }

    private static string Text(Value v, string method, int line, int col)
        => v.Kind == ValueKind.Str
            ? v.AsString
            : throw ScriptException.Error(DiagnosticCode.TypeMismatch,
                $"{method}() expects str arguments, got {v.KindName}", line, col);

    private static int Width(Value v, string method, int line, int col)
        => v.Kind == ValueKind.Num
            ? Math.Clamp((int)v.AsNumber, 0, MaxPadWidth)
            : throw ScriptException.Error(DiagnosticCode.TypeMismatch,
                $"{method}() width must be a number, got {v.KindName}", line, col);

    private static char Fill(Value[] a, string method, int line, int col)
    {
        if (a.Length < 2) return ' ';
        if (a[1].Kind != ValueKind.Str || a[1].AsString.Length != 1)
            throw ScriptException.Error(DiagnosticCode.TypeMismatch,
                $"{method}() fill character must be a single-character str", line, col);
