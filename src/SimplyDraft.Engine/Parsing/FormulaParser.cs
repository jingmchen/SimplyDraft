...
public static class FormulaParser
{
    ...
    )
    {
        ...
        foreach (var entry in mergedRawValues)
        {
            ...
            if (rawValue.Length > 1
                && rawValue[0] == ScriptingConstants.Formula.EscapeChar
                && (rawValue[1] == ScriptingConstants.Formula.Prefix
                || rawValue[1] == ScriptingConstants.Formula.EscapeChar))
            {
    ...
    )
    {
        ...
        // A formula that failed to parse (preview mode) must not stall its dependents into a fake E304
        //      — treat it as resolved; its value stays "".
        ...
        while (madeProgress)
        {
            ...
            foreach (var name in expressions.Keys.ToList())
            {
                ...
                try
                {
                    variables[name] = interpreter.Eval(expressions[name]);
                }
                catch (ScriptException ex)
                {
                    result.Diagnostics.Add(new Diagnostic(ex.Diagnostic.Code, DiagnosticSeverity.Error,
                        $"in formula for {{{name}}}: {ex.Diagnostic.Message}",
                        ex.Diagnostic.Line, ex.Diagnostic.Col));
                    if (!isPreview) return false;
                    variables[name] = Value.Str("");
                }
                catch (Exception ex)
                {
                    result.Diagnostics.Add(new Diagnostic(DiagnosticCode.SyntaxError, DiagnosticSeverity.Error,
        ...
        {
            result.Diagnostics.Add(new Diagnostic(DiagnosticCode.CircularFormula, DiagnosticSeverity.Error,
    ...
    )
    {
        foreach (var entry in types)
        {
            ...
            if (!isValid)
            {
                ...
                diagnostics.Add(new Diagnostic(DiagnosticCode.DeclaredTypeMismatch, DiagnosticSeverity.Warning,
    ...
    private static void CollectVarRefs(Expression root, List<string> referencedNames)
    {
        ...
        while (pending.Count > 0)
        {
            ...
            if (expression is NameExpression nameExpr) referencedNames.Add(nameExpr.Name);
